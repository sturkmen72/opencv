...
TEST(Imgcodecs_WebP, load_save_multiframes)
{
    ...
    string output = root + "readwrite/OpenCV_logo_white.webp";
    EXPECT_EQ(true, imwrite(output, png_frames));
    string avifoutput = root + "readwrite/OpenCV_logo_white.avif";
    EXPECT_EQ(true, imwrite(avifoutput, png_frames));
    string tifoutput = root + "readwrite/OpenCV_logo_white.tif";
    EXPECT_EQ(true, imwrite(tifoutput, png_frames));
    ...
    EXPECT_EQ(17, imcount(avifoutput)); //TO DO : actual return value is 1. should be frames count
    EXPECT_EQ(0, remove(output.c_str()));
}
#endif // HAVE_WEBP
