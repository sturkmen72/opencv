...
PERF_TEST_P(Encode, animation, testing::ValuesIn(exts_multi))
{
    ...
    TEST_CYCLE() imencodeanimation(GetParam(), animation, buf);

    std::cout << "  Encoded buffer size: " << buf.size() << std::endl;
...
PERF_TEST_P(Encode, multi_page, testing::ValuesIn(exts_multi))
{
    ...
    TEST_CYCLE() imencodemulti(GetParam(), animation.frames, buf);
