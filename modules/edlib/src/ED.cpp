...
void ED::ComputeGradient()
{
	...
	for (int j = 0; j<width; j++) { gradImg[j] = gradImg[(height - 1)*width + j] = (short)gradThresh - 1; }
	for (int i = 1; i<height - 1; i++) { gradImg[i*width] = gradImg[(i + 1)*width - 1] = (short)gradThresh - 1; }
