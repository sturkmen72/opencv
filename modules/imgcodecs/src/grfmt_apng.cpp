...
namespace uc {
    namespace apng {
        ...
        struct stbi_deleter
        {
            void operator()(uchar* v) const noexcept
            {
                free(v);
            }
        };
        using stbi_ptr = std::unique_ptr<uchar, stbi_deleter>;
        ...
        public:
            ...
            image_t(const image_t& obj)
                : width_(obj.width_), height_(obj.height_), bin(static_cast<uchar*>(malloc(size())))
            {
            ...
            image_t(uint32_t w, uint32_t h)
                : width_(w), height_(h), bin(static_cast<uchar*>(malloc(size())))
            {
