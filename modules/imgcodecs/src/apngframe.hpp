...
namespace uc {
    namespace apng {
        ...
        struct apng_deleter
        {
            void operator()(unsigned char* v) const noexcept
            {
                free(v);
            }
        };

        using apng_ptr = std::unique_ptr<unsigned char, apng_deleter>;
        ...
        public:
            ...
            image_t(const image_t& obj)
                : width_(obj.width_), height_(obj.height_), bin(static_cast<unsigned char*>(malloc(size())))
            {
            ...
            image_t(uint32_t w, uint32_t h)
                : width_(w), height_(h), bin(static_cast<unsigned char*>(malloc(size())))
            {
            ...
            {
                int w, h, d;
                bin = apng_ptr(apng_load_from_memory(pngBinData.data(), static_cast<int>(pngBinData.size()), &w, &h, &d, 4));
                width_ = w;
                height_ = h;
        ...
        public:
            ...
            frame next_frame()
            {
                ...
                printf("frameIndex: %zd delay_num %d delay_den %d IDATLoaded %d", frameIndex++, fcTLpayload.delay_num, fcTLpayload.delay_den, IDATLoaded);
